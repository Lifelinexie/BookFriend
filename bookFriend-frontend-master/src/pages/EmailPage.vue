<template>
  ...
  <van-tabs v-model:active="active" @change="onTabChange" swipeable>
  ...
  <div class="friend-list">
    <div class="friend-group">
      <ul class="friend-items" v-if="active==='1'">
        ...
        <li v-else v-if="status.applyFriends?.length > 0" v-for="friend in status.applyFriends" :key="friend.id"
            class="friend-item">
      ...
      <ul class="friend-items" v-else>
        ...
        <li v-else v-if="status.applyFriends?.length > 0" v-for="friend in status.myApplyFriends" :key="friend.id"
            class="friend-item">
          <div class="avatar-container">
...
const onTabChange = async (name: string) => {
  var ans;
  if (name === "1") {
    ans = await myAxios.get("/application/getRecords")
    status.value.applyFriends = ans.data;
  }
  if (name === "2") {
    ans = await myAxios.get("/application/getMyRecords")
    status.value.myApplyFriends = ans.data;
  }
}
const agree = async (id: number, fid: number) => {
  const agree = await myAxios.post(`/application/agree/${id}`)
  if (agree) {
...
const canceledApply = async (id: number) => {
  const canceledApply = await myAxios.post(`/application/canceledApply/${id}`)
  if (canceledApply) {
